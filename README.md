# 🌿 Sprout & Spade — GA4 / GTM Training Store

A deliberately simple **single-page app (SPA)** ecommerce website in the
**gardening niche**, built with **HTML, CSS, vanilla JS and PHP**. Everything
...
once `window.google_tag_manager['GTM-PQ8X4LR']` exists. A ~10s fallback fires
...
| `view_article_list` / `select_article` / `view_article` / `article_scroll` | The Potting Bench (blog) |
| `view_location_list` / `select_location` / `view_location` / `get_directions` / `click_to_call` | Store locator |
| `quiz_start` / `quiz_complete` | The Garden quiz |
...
- **Coupons** — `SPROUT10` (10% off) and `WELCOME5` ($5 off). The `coupon`
...
- **Documents** — product specs, care guides and store guides, generated as real
...
blog.php / blog-post.php  The Potting Bench
...
```php
$GTM_ID = 'GTM-PQ8X4LR';   // ← this store's container ID
```
